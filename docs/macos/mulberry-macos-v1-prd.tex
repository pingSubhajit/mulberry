...
\begin{itemize}
  ...
  \item Bootstrap must expose platform-aware partner presence while preserving the existing Android \texttt{partnerWallpaperStatus} contract for Android clients.
  \item Android wallpaper visibility reporting and \texttt{PARTNER\_VISIBILITY\_CHANGED} push payloads must remain backward compatible. Their current behavior must not regress: no push on the first-ever ``can see'' report, and notify only after a previously visible partner loses or regains Android wallpaper visibility.
  \item macOS overlay status must be reported as a separate \texttt{MACOS\_OVERLAY} surface keyed by a stable device instance. During Phase 5, macOS may report configured/enabled metadata, but it must not mark \texttt{canSeeLatestDrawings} true until canvas rendering and live or recovered sync state are implemented.
\end{itemize}
...
Phase 5 also establishes the backend presence-surface contract. Add the database/API groundwork for per-device \texttt{ANDROID\_WALLPAPER} and \texttt{MACOS\_OVERLAY} visibility, compute an aggregate ``can see latest drawings'' state across all configured surfaces, and preserve the existing Android wallpaper bootstrap and push behavior as a compatibility layer.

\textbf{Acceptance:} the app displays real account, partner, pair, streak, and partner presence state from the backend. Existing Android partner-visibility notifications and {\footnotesize\texttt{partnerWallpaperStatus}} semantics remain compatible.
