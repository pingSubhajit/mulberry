...
\begin{itemize}
  ...
  \item \texttt{PUT /me/presence-surfaces/:surfaceType}
...
presence\_surfaces & latest local view of own and partner Android wallpaper and macOS overlay presence. \\
...
\subsection{Presence Surfaces}

Add a platform-aware presence-surface model rather than adding user-level \texttt{overlayConfigured} columns. Presence is per user, pair session, device instance, and surface type. Required surface types for v1 are:
\begin{itemize}
  \item \texttt{ANDROID\_WALLPAPER}
  \item \texttt{MACOS\_OVERLAY}
\end{itemize}

The table should support at least:
\begin{itemize}
  \item user ID;
  \item pair session ID;
  \item device instance ID;
  \item surface type;
  \item configured state;
  \item enabled or visible state;
  \item \texttt{canSeeLatestDrawings};
  \item \texttt{hasEverBeenAbleToSee};
  \item details JSON for surface-specific facts such as Android home/lock selection or macOS display/overlay visibility;
...
The existing Android wallpaper contract must remain backward compatible while the new presence table is introduced:
\begin{itemize}
  \item keep {\footnotesize\texttt{user\_wallpaper\_status}};
  \item keep Android {\footnotesize\texttt{PUT /me/wallpaper-status}};
  \item keep bootstrap {\footnotesize\texttt{partnerWallpaperStatus}};
  \item keep the {\footnotesize\texttt{PARTNER\_VISIBILITY\_CHANGED}} push payload and timing.
\end{itemize}

Android wallpaper status writes should also update the {\footnotesize\texttt{ANDROID\_WALLPAPER}} presence surface so the aggregate model stays current.

macOS reports \texttt{MACOS\_OVERLAY} using a stable installation/device instance ID. Phase 5 may report configured and enabled metadata once the user has authenticated and bootstrap has completed, but it must not report {\footnotesize\texttt{canSeeLatestDrawings = true}} until the app can render the latest canvas through Phase 6 and stay current through Phase 7 sync/recovery.
...
Add a surface-aware notification payload, {\footnotesize\texttt{PARTNER\_PRESENCE\_CHANGED}}, for future clients that understand cross-platform presence. Do not replace the Android {\footnotesize\texttt{PARTNER\_VISIBILITY\_CHANGED}} payload in v1; Android notifications for wallpaper visibility must continue to fire with the legacy fields and timing.
...
\begin{itemize}
  ...
  \item Backend presence-surface aggregation and legacy Android wallpaper compatibility.
...
\begin{enumerate}
  ...
  \item Partner visibility is surface-aware: aggregate visibility is true when any configured active surface can show latest drawings, while Android wallpaper compatibility remains intact.
\end{enumerate}
